...
function populateForm(supplier: Supplier) {
  form.name = supplier.name
  form.address = supplier.address || ''
  form.post = supplier.post || ''
  form.city = supplier.city || ''
  form.land = supplier.land || ''
...
async function save() {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Fehler beim Aktualisieren des Lieferanten'
    toast.add({
      title: 'Fehler',
      description: message,
      color: 'error',
      ...
      <UCard class="w-150 max-w-full">
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold">Lieferant bearbeiten</h3>
            <UButton
              color="neutral"
              variant="ghost"
              icon="i-lucide-x"
