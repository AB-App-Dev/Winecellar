...
async function save() {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Fehler beim Erstellen des Weinguts'
    toast.add({
      title: 'Fehler',
      description: message,
      color: 'error',
      ...
      <UCard class="w-150 max-w-full">
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold">Neues Weingut hinzufügen</h3>
            <UButton
              color="neutral"
              variant="ghost"
              icon="i-lucide-x"
